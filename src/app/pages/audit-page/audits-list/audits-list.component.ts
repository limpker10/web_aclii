...
@Component({
    ...
    imports: [MatDialogModule, MatCardModule, MatInputModule, MatButtonModule, MatMenuModule, MatPaginatorModule, MatTableModule, MatTooltipModule, MatIcon, MatFormFieldModule, MatSortModule, MatTooltipModule, DatePipe],
...
export class AuditsListComponent implements OnInit, AfterViewInit {
    displayedColumns: string[] = ['index','code', 'entity_type', 'coordinator', 'auditor', 'applicable_standard', 'status', 'created_at','action'];
    ...
    loadUsers(page: number = 1, pageSize: number = 10): void {
        this.auditService.getAudits(page, pageSize).subscribe({
            next: (data) => {
                console.log(data)
